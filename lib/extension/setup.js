...
const getSettingsWithInitializedSecrets = async (settings) => {
  if (typeof settings !== 'object' || settings === null) {
    return settings;
  }

  if (settings.from_secret) {
    return await appConfigLoader.getSecret(settings.from_secret);
  }

  const updatedSettings = Array.isArray(settings) ? [] : {};

  for (const settingKey of Object.keys(settings)) {
    updatedSettings[settingKey] = await getSettingsWithInitializedSecrets(settings[settingKey]);
  }
    ...
    try {
      this.configuration.load({
        ...
        query: envConfigFilePath,
          ...
          return getSettingsWithInitializedSecrets(settings);
